...
export async function getAllSalesReq() {
  try {
    const url = createUrl("/inq/allsales");
    const response = await axios.get(`${url}`);
    return response.data;
  } catch (ex) {
    return createError(ex);
...
export async function onAddSale(id, CarID, CustomerID, EmployeeID, SalePrice) {
  const body = {
    id,
    CarID,
    CustomerID,
