...
export function Cart() {
 ...
 if(result['message'] == 'success')
  ...
  else {toast.error(result['result'].sqlMessage); console.log(result.result)}
    ...
    <AdminNavbar></AdminNavbar>
      <div className='container'>
        ...
        {/* conditional rendering Make,Model,Year,VIN,Price,Status,Image*/}
          <div className='card table-responsive card-body' style={{ minHeight:140 }}>
                ...
                total.map(item => {
                  return (
                    <tr key={item.CarID}>
                      ...
                      <Select options={options}  placeholder={item.Status} onChange={(selectedOption)=> onStatusChange(item.CarID,selectedOption)} />
                        
                      <span style={{marginLeft:'8px'}}><button onClick={() => ondeleteCar(item.CarID)}>deleteCar</button></span>
                      </td>
                    </tr>
                  )
                })}
